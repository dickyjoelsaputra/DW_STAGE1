...
func blogIndex(w http.ResponseWriter, r *http.Request) {
		...
		rows, _ := connection.Conn.Query(context.Background(), "SELECT id, title, description, technologies FROM tb_blog")
		...
		for rows.Next() {
			...
			err := rows.Scan(&each.Id, &each.Title, &each.Description, &each.Technologies ,)
			if err != nil {
				fmt.Println(err.Error())
				return
			}
...
func blogShow(w http.ResponseWriter, r *http.Request) {
	...
	err = connection.Conn.QueryRow(context.Background(), "SELECT id, title, description , technologies FROM tb_blog WHERE id=$1", id).Scan(
		&BlogDetail.Id, &BlogDetail.Title, &BlogDetail.Description, &BlogDetail.Technologies ,
	)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		w.Write([]byte("message : " + err.Error()))
...
func blogEdit(w http.ResponseWriter, r *http.Request) {
	...
	err = connection.Conn.QueryRow(context.Background(), "SELECT id, title, description FROM tb_blog WHERE id=$1", id).Scan(
		&BlogDetail.Id, &BlogDetail.Title, &BlogDetail.Description,
	)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		w.Write([]byte("message : " + err.Error()))
